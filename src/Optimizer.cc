...
#include "g2oAddition/include/EdgePlane.h"
#include "g2oAddition/include/VertexPlane.h"

#include <mutex>

namespace ORB_SLAM2 {
const float Optimizer::mDeltaMono = sqrt(5.991);
const float Optimizer::mDeltaStereo = sqrt(7.815);

double Optimizer::mPlaneAngleInfo = 0;    // 3282.8 / (ConfigFile::planeAngleInfo * ConfigFile::planeAngleInfo);
double Optimizer::mPlaneDistanceInfo = 0; //(ConfigFile::planeDistanceInfo * ConfigFile::planeDistanceInfo);
double Optimizer::mPlaneChi = 0;          // ConfigFile::planeChi;
double Optimizer::mDeltaPlaneChi = 0;     // sqrt(ConfigFile::planeChi);
bool Optimizer::mInitFlag = false;
...
int Optimizer::PoseOptimization(Frame* pFrame) {
    ...
    if (!mInitFlag) {
        ...
        mPlaneChi = ConfigFile::planeChi;
        mDeltaPlaneChi = sqrt(ConfigFile::planeChi);
        mInitFlag = true;
    }

    {
        ...
        // cout << "Frame plane num: " << pFrame->mnPlaneNum << endl;
        // cout << "Plane num: " << pFrame->mvpMapPlanes.size() << endl;
        for (int i = 0; i < M; ++i) {
            ...
            if (pMP) {
                ...
                if (vnVertexId.count(pMP->mnId) == 0) {
                    ...
                    long unsigned int id = pMP->mnId + 1;
                    if (id > maxPlaneid)
                        maxPlaneid = id;
                    vP->setId(id);
                    vP->setFixed(true);
...
void Optimizer::LocalBundleAdjustment(KeyFrame* pKF, bool* pbStopFlag, Map* pMap) {
    ...
    for (list<KeyFrame*>::iterator lit = lLocalKeyFrames.begin(), lend = lLocalKeyFrames.end(); lit != lend; lit++) {
        ...
        // TODO: Local MapPlanes seen in Local KeyFrames
        vector<MapPlane*> vpMapPlanes = (*lit)->GetMapPlanes();
        for (vector<MapPlane*>::iterator vit = vpMapPlanes.begin(), vend = vpMapPlanes.end(); vit != vend; vit++) {
            MapPlane* pMapPlane = *vit;
            if (pMapPlane)
                if (pMapPlane->mnBALocalForKF != pKF->mnId) {
                    lLocalMapPlanes.push_back(pMapPlane);
                    pMapPlane->mnBALocalForKF = pKF->mnId;
                }
        }
    }
    ...
    // TODO: Fixed Keyframes. Keyframes that see Local MapPlanes but that are not Local Keyframes
    ...
    unsigned long maxPointid = 0; // Find the last id of mappoint
    ...
    for (list<MapPoint*>::iterator lit = lLocalMapPoints.begin(), lend = lLocalMapPoints.end(); lit != lend; lit++) {
        ...
        if (id > maxPointid) { // Find the last id of mappoint
            maxPointid = id;
        }
        vPoint->setId(id);
        vPoint->setMarginalized(true);
    ...
    for (list<MapPlane*>::iterator lit = lLocalMapPlanes.begin(), lend = lLocalMapPlanes.end();
        ...
        for (map<KeyFrame*, size_t>::const_iterator mit = observations.begin(), mend = observations.end();
            ...
            if (!pKFi->isBad()) {
                ...
                e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(id)));
                e->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(pKFi->mnId)));
                e->setMeasurement(Converter::toPlane3D(pKFi->mvPlaneCameraCoefficients[mit->second].mPlaneCameraCoefficient));
                Eigen::Matrix3d Info;
                Info << mPlaneAngleInfo, 0, 0,
                    0, mPlaneAngleInfo, 0,
                    0, 0, mPlaneDistanceInfo;
    ...
    if (bDoMore) {
        ...
        // TODO: Check plane inlier observations
        for (size_t i = 0, iend = vpEdgesPlane.size(); i < iend; i++) {
            g2o::EdgePlane* e = vpEdgesPlane[i];
    ...
    // TODO: Check inlier observations
    ...
    for (list<MapPlane*>::iterator lit = lLocalMapPlanes.begin(), lend = lLocalMapPlanes.end(); lit != lend; lit++) {
        ...
        bool success = pMP->TransformPointCloud(Converter::toVector4d(pMP->GetWorldPos()), Converter::toVector4d(Converter::toCvMat(vPlane->estimate())));
        if (success) {
