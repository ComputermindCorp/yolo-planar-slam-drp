...
KeyFrame::KeyFrame(Frame& F, Map* pMap, KeyFrameDatabase* pKFDB)
    : mnFrameId(F.mnId), mTimeStamp(F.mTimeStamp), mnGridCols(FRAME_GRID_COLS), mnGridRows(FRAME_GRID_ROWS),
      ...
      mnMaxY(F.mnMaxY), mK(F.mK), mbToBeErased(false), mbNewPlane(F.mbNewPlane), mnPlaneNum(F.mnPlaneNum), mvPlaneCameraCoefficients(F.mvPlaneCameraCoefficients),
      mvpMapPoints(F.mvpMapPoints), mvpMapPlanes(F.mvpMapPlanes), mpKeyFrameDB(pKFDB),
...
std::vector<MapPlane*> KeyFrame::GetMapPlanes() {
    unique_lock<mutex> lock(mMutexFeatures);
    return mvpMapPlanes;
}

void KeyFrame::AddMapPlane(MapPlane* pMP, const int& idx) {
    unique_lock<mutex> lock(mMutexFeatures);
    if (idx < (long long int)mvpMapPlanes.size() && idx >= 0) {
        mvpMapPlanes[idx] = pMP;
    }
    else {
        cout << "Add MapPlane index out of range" << endl;
    }
    return;
}
...
void KeyFrame::EraseMapPlaneMatch(const int& idx) {
    ...
    if (idx < (long long int)mvpMapPlanes.size() && idx >= 0) {
        ...
        cout << "Erase MapPlane index out of range" << endl;
    }
    return;
}
...
void KeyFrame::EraseMapPlaneMatch(MapPlane* pMP) {
    int idx = pMP->GetIndexInKeyFrame(this);
    if (idx >= 0 && idx < (long long int)mvpMapPlanes.size()) {
        mvpMapPlanes[idx] = static_cast<MapPlane*>(NULL);
    }
    return;
}
...
void KeyFrame::BuildMapPlanePointCloud(const cv::Mat& imDepth, const cv::Mat& segmentImage, int cloudStep) {
