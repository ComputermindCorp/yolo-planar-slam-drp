...
// map_segment.pb.h will be generated into build/ when make
#include "map_segment.pb.h"

namespace socket_publisher {

std::string DataSerializer::serialized_reset_signal_{};
...
DataSerializer::DataSerializer(
    ...
    : pFrameDrawer_(pFrameDrawer), pMapDrawer_(pMapDrawer),
    ...
    DataSerializer::serialized_reset_signal_ = serialize_messages(tags, messages);
}

std::string DataSerializer::serialize_messages(const std::vector<std::string>& tags, const std::vector<std::string>& messages) {
...
std::string DataSerializer::serialize_map_diff() {
    std::vector<ORB_SLAM2::KeyFrame*> keyframes = pMapDrawer_->mpMap->GetAllKeyFrames();

    std::vector<ORB_SLAM2::MapPoint*> all_landmarks = pMapDrawer_->mpMap->GetAllMapPoints();
    std::vector<ORB_SLAM2::MapPoint*> local_landmarks = pMapDrawer_->mpMap->GetReferenceMapPoints();

    const auto current_camera_pose = pMapDrawer_->GetCurrentCameraPose();
...
std::string DataSerializer::serialize_latest_frame(const unsigned int image_quality, const double track_time) {
    bool drawPlaneSegment = false;
    const auto image = pFrameDrawer_->DrawFrame(drawPlaneSegment, track_time);
...
std::string DataSerializer::serialize_as_protobuf(
    ...
    for (const auto keyfrm : keyfrms) {
        if (!keyfrm || keyfrm->mbToBeErased) {
            continue;
        }

        const auto id = keyfrm->mnId;
        const auto pose = keyfrm->GetPose();
        ...
        keyfrm_obj->set_id(keyfrm->mnId);
        map_segment::map_Mat44* pose_obj = new map_segment::map_Mat44();
        for (int i = 0; i < 16; i++) {
            int ir = i / 4;
            int il = i % 4;
            pose_obj->add_pose(pose.at<float>(ir, il));
        }
    ...
    *keyframe_hash_map_ = next_keyframe_hash_map;
    ...
    for (const auto keyfrm : keyfrms) {
        ...
        const unsigned int keyfrm_id = keyfrm->mnId;

        // covisibility graph
        const auto covisibilities = keyfrm->GetCovisiblesByWeight(100);
        if (!covisibilities.empty()) {
            for (const auto covisibility : covisibilities) {
                if (!covisibility || covisibility->mbToBeErased) {
                    continue;
                }
                if (covisibility->mnId < keyfrm_id) {
                    continue;
                }
        ...
        // spanning tree
        auto spanning_parent = keyfrm->GetParent();
        if (spanning_parent) {
            const auto edge_obj = map.add_edges();
            edge_obj->set_id0(keyfrm_id);
        ...
        // loop edges
        const auto loop_edges = keyfrm->GetLoopEdges();
    ...
    for (const auto landmark : all_landmarks) {
        if (!landmark || landmark->mbBad) {
            continue;
        }

        const auto id = landmark->mnId;
        const auto pos = landmark->GetWorldPos();
        ...
        for (int i = 0; i < 3; i++) {
            landmark_obj->add_coords(pos.at<float>(i));
        }
        for (int i = 0; i < 3; i++) {
    ...
    *point_hash_map_ = next_point_hash_map;
    ...
    for (const auto landmark : local_landmarks) {
        map.add_local_landmarks(landmark->mnId);
    }
    ...
    for (int i = 0; i < 16; i++) {
        int ir = i / 4;
        int il = i % 4;
        pose_obj.add_pose(current_camera_pose.at<float>(ir, il));
    }
...
std::string DataSerializer::base64_encode(unsigned char const* bytes_to_encode, unsigned int in_len) {
